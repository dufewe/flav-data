...
- **Phenomenological fits** — interpretations, not primary measurements.
- **Conference papers** — results without a formal preprint or publication.
- **Report papers** — conference contributions or internal reports without peer review.
...
**How to access**:
```bash
# hepdata-cli install + lookup: see ./hepdata-cli.md.
# Briefly: install with `pip install hepdata-cli`; the importer resolves
# the binary at runtime from $PATH / venv bin / ~/.local/bin.
...
### Metadata Source (parallel, not sequential)

**InspireHEP API** provides paper metadata (title, authors, collaboration, TexKey, DOI) but NOT experimental data. Query it in parallel with steps 1–6 for every paper:

- **Why**: metadata (not data values) is always needed.
- **How**: query by arXiv ID, recid, TexKey, or DOI (see `../references/inspirehep-api.md`).
...
See SKILL.md "Import Conventions" — the 4 rules there (matrix layout,
single-line abstract, InspireHEP author format, and LaTeX-only Unicode)
